...
description: >
  ...
  accent, the three-step gray text ladder, SF Pro Display (display/numerals/chrome) + SF Pro Display (body),
...
colors:
  # Hand-corrected after build-frame: the remix put the brand accent on text-muted and white on
  # the canvas. Values below are Foldy's own site tokens (--bg, --accent, --ink, --muted).
  bg: "#ECEDF2"
  card: "#FFFFFF"
  primary: "#0A84FF"
  primary-deep: "#0062D6"
  ...
  accent-medium: "rgba(10, 132, 255, 0.15)"
  border: "rgba(14, 18, 32, 0.085)"
...
typography:
  # — reading ramp (SF Pro Display body + SF Pro Display chrome) —
  body:    { fontFamily: "SF Pro Display", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "SF Pro Display", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "SF Pro Display", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "SF Pro Display", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (SF Pro Display, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "SF Pro Display", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "SF Pro Display", cqw: 1.9, weight: 600, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "SF Pro Display", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "SF Pro Display", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "SF Pro Display", cqw: 3.0, weight: 600, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "SF Pro Display", cqw: 4.2, weight: 600, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "SF Pro Display", cqw: 8.0, weight: 600, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
### Foldy overrides (hand-written; these win over the preset prose)

- **Font.** SF Pro ships with the project as the variable file `assets/fonts/SFNS.ttf` (the macOS
  system face; optical size switches Text/Display automatically). Declare it once per frame:
  `@font-face { font-family: "SF Pro"; src: url("assets/fonts/SFNS.ttf") format("truetype"); font-weight: 1 1000; font-display: block; }`
  and use `font-family: "SF Pro", sans-serif` for every role — wherever the ramp below says
  "SF Pro Display" / "SF Pro Text", write "SF Pro". Display weight 600, tracking −0.02em.
- **Ground.** `{colors.bg}` #ECEDF2 with the site's soft ambient washes (blue rgba(104,152,255,.34)
  top-left, peach rgba(255,186,150,.26) top-right, teal rgba(132,216,209,.28) bottom) as large blurred
  radial gradients. The opening frame may sit on pure black (#000) so the full-bleed fold falls into
  darkness, then hands over to the light ground.
- **Surfaces are glass, not cobalt tint.** Pills and chips: white 50–72% fill, 1px white rim at 95%,
  `backdrop-filter: blur(20px)`, radius 100px — the site's Liquid Glass. Cards `{colors.card}` white,
  radius 22–28px, one soft shadow `0 30px 80px rgba(24,32,56,.18)` for the MacBook stage only.
- **The product is the image.** The 3D MacBook renders (`assets/app/*`) are the focal of every frame
  that shows the fold; typography supports it, never competes. No dashboards, no bar charts, no
  invented numbers — the only figures allowed are the real ones: 135°, macOS 14, 2.2 MB, free.
